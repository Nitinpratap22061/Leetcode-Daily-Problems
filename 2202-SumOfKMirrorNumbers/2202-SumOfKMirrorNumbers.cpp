// Last updated: 10/5/2025, 8:53:14 AM
class Solution {
public:


      bool isPalindrome(string s)
      {
        int i =0 ; 
        int j = s.length()-1 ;
        while(i<j)
        {
            if(s[i]!=s[j])
            {
                return false;
            }
            i++;
            j--;
        }
        return true ;
      }
     string converttobasek(long long num , int k)
     {
        if(num==0)
        {
            return "0";
        }
        string res = "";
        while(num>0)
        {
            res+=to_string(num%k);
            num = num/k;
        }
        return res ;
     }
    long long kMirror(int k, int n) {
        long long sum = 0 ; 
        int L=1;
        while(n>0)
        {
            int half_length = (L+1)/2 ;
            long long mini = pow(10,half_length-1);
            long long maxi = pow(10,half_length)-1 ; 
            for(int num = mini ; num<=maxi ; num++)
            {
                string first_half = to_string(num);
                string second_half = first_half ; 
                reverse(begin(second_half) , end(second_half));
                string pal = "";
                if(L%2==0)
                {
                    pal = first_half+second_half ;
                }
                else
                {
                    pal = first_half + second_half.substr(1);
                }

                long long palindromedec = stoll(pal) ; 
                string basek = converttobasek(palindromedec , k) ; 
                if(isPalindrome(basek))
                {
                    sum+=palindromedec ; 
                    n-- ; 
                    if(n<=0){
                      break ;
                    }
                }
            }
            L++;
        }
        return sum;
    }
};