...
public:
    ...
    int countUnguarded(int m, int n, vector<vector<int>>& guards, vector<vector<int>>& walls) {
        ...
        // for each guard → call 4 directions
        for (auto &g : guards) {
            int i = g[0], j = g[1];
            solve(grid, i, j, m, n, 1, 0);   // down
            solve(grid, i, j, m, n, -1, 0);  // up
            solve(grid, i, j, m, n, 0, 1);   // right
            solve(grid, i, j, m, n, 0, -1);  // left
        }
